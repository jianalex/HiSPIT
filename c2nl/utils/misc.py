...
def relative_matmul(x, z, transpose):
    ...
    x_t_r = x_t.reshape(length, heads * batch_size, -1)#13,8,64 #77,8,64
    if transpose:
        z_t = z.transpose(1, 2)#13,64,13 #13,64,13
        x_tz_matmul = torch.matmul(x_t_r, z_t) #13,8,13
    else:
        x_tz_matmul = torch.matmul(x_t_r, z)
    x_tz_matmul_r = x_tz_matmul.reshape(length, batch_size, heads, -1)#13,1,8,13 #77,1,8,64
    x_tz_matmul_r_t = x_tz_matmul_r.permute(1, 2, 0, 3)
    return x_tz_matmul_r_t #1,8,13,13 #1,8,77,64
