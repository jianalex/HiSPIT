...
from c2nl.modules.embeddings import Embeddings, Gau2Embedder
...
class Encoder(nn.Module):
    ...
    def forward(self,
                input,
                input_len,
                adjacency, # for code structure 
                code_struc_code_rep
                ):
        # input = input + self.gau(input)
...
class Transformer(nn.Module):
    ...
    def _run_forward_ml(self,
        ...
        code_struc_code_rep = kwargs['code_struc_code_rep']

        memory_bank, layer_wise_outputs = self.encoder(code_rep, 
                                                    code_len, 
                                                    code_struc_rep, # for code structure 
                                                    code_struc_code_rep
                                                    )  # B x (seq_len+node_num) x h
        
        memory_bank_copy = memory_bank[:, :code_rep.size(1),:]
        memory_len = code_len# + code_pdg_node_num_rep 
        ...
        layer_wise_dec_out, attns = self.decoder(enc_outputs,
                                                 memory_len, # for pdg #code_len, 
                                                 summ_pad_mask,
                                                 summ_emb) # layer x B x tgt_len x h
        ...
        if self._copy:
            ...
            _, copy_score, _ = self.copy_attn(decoder_outputs,
                                              memory_bank_copy,# for pdg #memory_bank,
    ...
    def __generate_sequence(self,
        ...
        dec_states = self.decoder.init_decoder(params['memory_len'], max_mem_len) # for pdg
    ...
    def decode(self,
        ...
        memory_bank, layer_wise_outputs = self.encoder(word_rep, 
                                                    code_len, 
                                                    code_struc_rep, # for code structure 
                                                    code_struc_code_rep
                                                    )  # B x (seq_len+node_num) x h
        ...
        # for pdg
        params['memory_len'] = memory_len 
        params['memory_bank_copy'] = memory_bank[:, :word_rep.size(1),:]
