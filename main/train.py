...
def add_train_args(parser):
    ...
    # for code structure _ AST
    files.add_argument('--train_guid', nargs='+', type=str, default=['train.token.guid'],
    ...
    if not args.only_test:
        ...
        for i in range(num_dataset):
            ...
            if args.use_code_struc:
                ...
                if not os.path.isfile(train_guid):
                    raise IOError('No such file: %s' % train_guid)
            else:
                train_guid = None
...
def train(args, data_loader, model, global_stats):
    ...
    for idx, ex in enumerate(pbar):
        bsz = ex['batch_size']
        if args.optimizer in ['sgd', 'adam', 'adamW'] and current_epoch <= args.warmup_epochs:
...
def validate_official(args, data_loader, model, global_stats, mode='dev'):
    ...
    if mode == 'test':
        ...
        ### for draw graph ###
        # def draw(data, x, y, ax, srclen, square = True):
        #     print(data.size())
        #     seaborn.heatmap(data[:srclen, :srclen], xticklabels = x[:srclen], square = square, 
        #             yticklabels = y[:srclen], vmin = 0.0, vmax = 1.0, cbar = False, ax=ax,
        #             linewidths = 0.5, linecolor = 'black', cmap = 'Blues')
        #     ax.set_xticklabels(x[:srclen], fontdict={'fontsize':12})
        #     ax.set_yticklabels(y[:srclen], fontdict={'fontsize':12})
        # code_id = 624
        # bach_id = 16
        # print(sources[code_id])
        # srclen = min(len(sources[code_id].split()), 150)
        # tgtlen = 60
        # print(srclen, tgtlen)
        # for layer in range(0, 3):
        #     fig, axs = plt.subplots(1, 1, figsize = (20,20))
        #     print('Encoder v_layer', layer+1)
        #     draw(model.network.encoder.transformer.v_layer[layer].attention.attn[0][bach_id].cpu().data, 
        #                 sources[code_id].split(), sources[code_id].split(), srclen = srclen, ax=axs)
        #     plt.savefig(str(code_id)+'_Encoder_v_layer_'+str(layer+1)+'.png')

        #     fig, axs = plt.subplots(1, 1, figsize = (20, 20))
        #     print('Encoder s_layer', layer+1)
        #     tree_node_id = [i for i in range(1, tgtlen+1)]
        #     draw(model.network.encoder.transformer.s_layer[layer].attention.attn[0][bach_id].cpu().data, 
        #                 tree_node_id, tree_node_id, srclen = tgtlen, ax=axs)
        #     plt.savefig(str(code_id)+'_Encoder_s_layer_'+str(layer+1)+'.png')
        
        # for layer in range(1, 6, 2):
        #     fig, axs = plt.subplots(1, 1, figsize = (50,50))
        #     print('Decoder Src layer', layer+1)
        #     draw(model.network.decoder.transformer.layer[layer].attention.attn[0][5].cpu().data, 
        #                sources[code_id].split(), hypotheses[code_id][0].split(), ax=axs, square = False)
        #     plt.savefig(str(code_id)+'_Decoder_Src_layer_'+str(layer+1)+'.png')
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src_tag, train_tgt, train_guid, dataset_name in \
                zip(args.train_src_files, args.train_src_tag_files,
                    args.train_tgt_files, args.train_guid_files, args.dataset_name):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_tag'] = train_src_tag
            train_files['tgt'] = train_tgt
            # for code structure _ AST
            train_files['guid'] = train_guid
    ...
    # TRAIN/VALID LOOP
        ...
        if args.optimizer in ['sgd', 'adam', 'adamW'] and args.warmup_epochs >= start_epoch:
        ...
        for epoch in range(start_epoch, args.num_epochs + 1):
            ...
            elif args.optimizer == 'adamW' and epoch > args.warmup_epochs:
                #model.optimizer.param_groups[0]['lr'] = \
                #       model.optimizer.param_groups[0]['lr'] * args.lr_decay
                if epoch >50 :
                    model.optimizer.param_groups[0]['lr'] = \
                        model.optimizer.param_groups[0]['lr'] * args.lr_decay * args.lr_decay
                else:
                    model.optimizer.param_groups[0]['lr'] = \
                        model.optimizer.param_groups[0]['lr'] * args.lr_decay
