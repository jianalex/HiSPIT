...
def process_examples(lang_id,
    ...
    if guid is not None:
        if lang_id == 0:
            # for pdg
            code.struc = np.load('../../data/java/adjacency_code_stmt/{}.npy'.format(guid))
            code.struc_code = np.load('../../data/java/adjacency_code/{}.npy'.format(guid))
            
        else:
            code.struc = np.load('../../data/python/adjacency/{}.npy'.format(guid))
...
def load_data(args, filenames, max_examples=-1, dataset_name='java',
    ...
    for guid, src, src_tag, tgt in tqdm(zip(guids, sources, source_tags, targets),
                                  total=len(sources)):
        if dataset_name in ['java', 'python', 'java_deepcom']:
