...
class TreePositionalEncodings(torch.nn.Module):
    ...
    def __init__(self, depth=8, degree = 16, n_feat=512, d_model=512):
        """
...
class Gau2Embedder(nn.Module):
    ...
    def __init__(self, dim, depth = 3, sigma = 0.001, max_len=512, bias = True, dropout=0.2):
        if dim % 2 != 0:
            raise ValueError("Cannot use sin/cos positional encoding with "
                             "odd dim (got dim={:d})".format(dim))
        pe = torch.zeros(max_len, dim)
        self.sig = sigma
        self.dic = torch.linspace(0., 1, steps=int(max_len)+1)[:-1].reshape(1,-1)
        super(Gau2Embedder, self).__init__()
        ...
        if depth == 0:
            self.mm.append(nn.Linear(max_len, dim, bias=bias))
        else:
            self.mm.append(nn.Sequential(nn.Linear(max_len, max_len//2,bias=bias),nn.ReLU(True)))
            for i in range(depth-1):
                self.mm.append(nn.Sequential(nn.Linear(max_len//2, max_len//2, bias=bias),nn.ReLU(True)))
            self.mm.append(nn.Sequential(nn.Linear(max_len//2, dim,bias=bias)))
    ...
    def forward(self, emb, step=None):
        ...
        """
        if emb.is_cuda:
            self.dic = self.dic.cuda()
        emb = (-0.5*(emb-self.dic)**2/(self.sig**2)).exp()
...
class VecEmbedding(nn.Module):
    def __init__(self, vec_size,
        ...
        if self.position_encoding:
            self.pe = PositionalEncoding(dropout, self.embedding_size) #pdg3+
            # self.pe = Gau2Embedder(self.embedding_size, depth = 1, sigma = 0.003)#pdg3-
...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
        ...
        self.position_encoding = position_encoding
        if self.position_encoding:
            pe = PositionalEncoding(dropout, self.embedding_size) #pdg3+
            #pe = Gau2Embedder(self.embedding_size, depth = 2, sigma = 0.003, dropout=dropout) #pdg3-
